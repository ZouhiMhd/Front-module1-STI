...
import { 
  ...
  Microscope, Pill, Loader2, ArrowLeft, Menu, 
  AlertCircle
} from 'lucide-react'; // Ajout de ArrowLeft et Menu
...
// ... (Garder le composant CaseListItem tel quel) ...
const CaseListItem = ({ data, active, onClick }: { data: ClinicalCase, active: boolean, onClick: () => void }) => (
  <div 
    onClick={onClick}
...
export default function ClinicalReviewDashboard() {
  ...
  // NOUVEL ÉTAT : Pour gérer l'affichage mobile
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setCases(data);
        // Sur Desktop, on sélectionne le premier par défaut
        // Sur Mobile, on ne sélectionne rien pour laisser la liste visible
  ...
  const filteredCases = useMemo(() => {
    ...
    return cases.filter(c => 
      filterType === 'SYMPTOM' 
        ? c.consultation.reason.toLowerCase().includes(lowerQuery) || c.consultation.symptoms.some(s => s.location.toLowerCase().includes(lowerQuery))
        : c.history.chronicDiseases.some(d => d.name.toLowerCase().includes(lowerQuery))
    );
  }, [cases, searchQuery, filterType]);

  // GESTION DU CLIC SUR UN CAS
  const handleCaseClick = (id: string) => {
      setSelectedCaseId(id);
      setShowMobileDetail(true); // Bascule vers la vue détail sur mobile
  };

  // GESTION DU RETOUR ARRIÈRE (MOBILE)
  ...
  const updateCaseStatus = (id: string, status: any, reason?: string) => {
      setCases(prev => prev.map(c => c.id === id ? { ...c, status, rejectionReason: reason } : c));
  };

  if (isLoading) return <div className="h-screen flex items-center justify-center bg-slate-50 text-slate-400"><Loader2 className="animate-spin mr-2"/> Chargement...</div>;
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans text-slate-600 overflow-hidden relative">
      ...
      {/* =========================================================================
          ...
          - md:w-96 : Largeur fixe sur desktop
          - hidden md:flex : Caché sur mobile SI on regarde un détail (showMobileDetail=true), sinon visible
          - flex : Visible par défaut (si showMobileDetail=false)
      ========================================================================= */}
      <aside className={`
        ...
        <div className="flex-1 overflow-y-auto">
           {filteredCases.map(c => (
             <CaseListItem 
                ...
                onClick={() => handleCaseClick(c.id)} // Utilise le nouveau handler
             />
           ))}
        </div>
      </aside>
      ...
      {/* =========================================================================
          ...
          - hidden md:flex : Caché sur mobile SI on est en mode liste (showMobileDetail=false)
          - flex : Visible sur mobile SI showMobileDetail=true
      ========================================================================= */}
      <main className={`
          ...
          <>
            ...
            <header className="bg-white/80 backdrop-blur-md border-b border-gray-200 px-4 md:px-8 py-4 flex justify-between items-center sticky top-0 z-10 shrink-0">
               <div className="flex items-center gap-3">
                  {/* BOUTON RETOUR (Visible uniquement sur Mobile 'md:hidden') */}
                  <button 
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8">
               <div className="max-w-6xl mx-auto space-y-6">
                  ...
                  {/* ... (Reste du contenu inchangé : VitalsCard, PatientIdentityCard, etc.) ... */}
                  {/* Assure-toi juste que VitalsCard utilise la version corrigée fournie précédemment */}
                  ...
                  {/* Reste des sections (Consultation, etc)... Copier le contenu du composant précédent ici */}
                  {/* Pour l'exemple, j'abrége ici, mais tu gardes tout ton code de rendu existant */}
                  <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
                      ...
                      <p className="text-lg font-medium text-slate-800">{activeCase.consultation.reason}</p>
                      {/* ... suite des symptômes ... */}
                   ...
                   {/* Section Antécédents & Traitements */}
      ...
      {/* Modal Rejet (Doit être en absolute/fixed par dessus tout) */}
      {rejectModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 px-4">
           ...
           <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
              ...
              <div className="flex justify-end gap-3">
                  ...
                  <button onClick={handleRejectConfirm} className="px-4 py-2 bg-red-600 text-white rounded-lg">Rejeter</button>
              </div>
           </div>
        </div>
      )}
